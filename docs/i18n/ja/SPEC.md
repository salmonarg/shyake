...
Shyake は、POSIX スタイルのコマンドラインインターフェース
クライアントを備えた、耐量子・エンドツーエンド暗号化の非同期
メールシステムであり、検閲や監視に対抗するための分散型
コミュニケーション手段として設計されています。

主な特性：

- **エンドツーエンド暗号化**：サーバーは平文を一切保持しません。
  すべてのメッセージ内容は送信前にクライアント側で暗号化されます。
...
- **分散型**：どの運営者もほぼゼロコストで自分のインスタンスを
  ホストできます。インスタンスは、サーバー間リレーモデルを用いて
  任意でフェデレーションします。
- **ステートレスなサーバー**：サーバーは暗号文と公開鍵のみを
  保存します。
- **保存時に暗号化される鍵**：クライアントの秘密鍵は、任意で
  パスフレーズ（scrypt + ChaCha20-Poly1305）によりディスク上で
  保護されます。
...
- **ビルドシステム**：GNU Make；クロスプラットフォーム
  （macOS、GNU/Linux、Termux）
- **成果物**：
...
- **署名検証**：WebAssembly にコンパイルされた ML-DSA-65
  （`mldsa65-wasm`）を、Wrangler の `CompiledWasm` ルール経由で
  ロードします。
...
各ユーザーは `liboqs` を用いてローカルで 2 つの独立した鍵ペアを
生成します：
...
公開鍵は生バイトとして保存され、登録時にサーバーへアップロード
されます。秘密鍵は、パスフレーズが設定されている場合は §3.7 に
記述する保存時暗号化フォーマットで、設定されていない場合は
生バイトとして保存されます。
...
3. **受信者の ML-KEM 公開鍵**にカプセル化する：KEM カプセル化に
   より KEM 暗号文と 32 バイトの共有秘密が得られる。対称鍵を
   共有秘密と XOR して連結する：
   `enc_key_recipient = base64(kem_ct || (sym_key XOR ss))`。
4. **送信者自身の ML-KEM 公開鍵**に対して同じカプセル化を
   繰り返す → `enc_key_sender`（送信者が自分の送信ボックスを
   読めるようにするため）。

復号はこの逆の手順です：クライアントは自身の KEM 秘密鍵で共有
秘密をデカプセル化し、それを暗号化鍵フィールドと XOR して対称鍵
を復元し、その後コンテンツを復号します。

単体ファイル暗号化コマンド（`enc` / `dec`）は、同じ
ML-KEM-768 + ChaCha20-Poly1305 構成を、長さプレフィックス付きの
バイナリコンテナ（`.enc` ファイル）で使用します。

#### 3.3 認証プロトコル

認証を要するすべての操作は ML-DSA-65 で署名されます。2 つの
運搬形式が使われます：

**ヘッダーベース**（登録とメール送信を除くすべての認証
エンドポイント）：
...
署名対象のメッセージは、HTTP メソッド、エンドポイント
（クエリ文字列を含む）、ユーザー名、タイムスタンプから構成される
決定的な文字列です——例：
...
**ボディベース**（`POST /api/register` と `POST /api/mail`）：
署名と PoW トークンはリクエストボディの JSON フィールドとして
運ばれます。署名対象のメッセージは、以下のペイロード部分集合の
コンパクト JSON シリアライゼーションです（フィールド順は
クライアントが生成した通り）：
...
`enc_key_recipient`、`signature`、`pow` を運びますが、これらは
署名対象の部分集合には含まれません。

サーバーは、D1 に保存された送信者の `sig_pubkey`
（フェデレーションメールの場合は送信者のインスタンスから取得）を
用い、WASM ML-DSA モジュール経由で署名を検証します。

#### 3.4 リプレイ対策

署名対象のすべてのメッセージにタイムスタンプが含まれます。
サーバーは、タイムスタンプがサーバー時刻から **300 秒（5 分）**
...
`resource` は操作するユーザーのユーザー名です。トークンは
クライアント側で生成（マイニング）され、署名検証やデータベース
処理より前にサーバー側で検証されます。

#### 3.6 鍵フィンガープリント

フィンガープリントは、生の（デコード済み）ML-KEM 公開鍵バイトの
**SHA-256** を小文字 16 進エンコードしたものです。クライアントは
信頼済みの鍵を `~/.config/shyake/known_hosts` にキャッシュします。
...
クライアントは送信のたびに受信者の最新の鍵を `known_hosts` と
比較し、さらにペイロードに `recipient_kem_fingerprint` を
埋め込みます。これによりサーバーは保存済みの鍵と比較し、古い鍵に
基づく送信を `KEY_MISMATCH`（HTTP 409）で拒否できます。

#### 3.7 秘密鍵の保存時保護

ユーザーが空でないパスフレーズを設定すると、秘密鍵ファイル
（`kem_sk.bin`、`sig_sk.bin`）は `SHYK` コンテナフォーマットで
書き込まれます：
...
パラメータへのいかなる改ざんも認証に失敗します。KDF は
パスフレーズから 256 ビットの ChaCha20-Poly1305 鍵を導出します。

`SHYK` マジックを持たないファイルはレガシーな生鍵として扱われ、
そのままロードされます。空のパスフレーズの場合は生の
（暗号化されていない）鍵が書き込まれます。

パスフレーズは対話的に入力（ターミナルエコー無効）するか、
非対話用途では `SHYAKE_PASSPHRASE` 環境変数で渡します。
`rotate` は現在のパスフレーズと新しいパスフレーズの入力を求め、
新しい鍵ペアはサーバーがローテーションを確認した後にのみ、
新しいパスフレーズで保存されます。
...
`destroy` 時、`kem_pubkey` と `sig_pubkey` は空文字列に設定され、
そのユーザーに関わるすべてのメール行とブロック行が削除されます。
ユーザー行自体は、ユーザー名を永久にロックするために
**保持されます**。
...
のインデックスがメールボックスクエリを支えます。`rotate` は
そのユーザーが送信者または受信者であるすべてのメール行を
削除します。
...
メール送信時、受信者が送信者のローカル名または送信者のドメインを
ブロックしている場合、サーバーは HTTP 403 で送信を拒否します。

---

### 5. HTTP API

すべてのエンドポイントは Cloudflare Worker 上でホストされます。
ベース URL は設定された `INSTANCE_DOMAIN` です。
...
`/api/pubkey/:username` は `user@domain` 構文をサポートします。
ドメインがローカルインスタンスと異なる場合、サーバーはリクエスト
をリモートインスタンスへプロキシします（フェデレーションの有効化
が必要）。

`/api/client/version` は GitHub Releases API をプロキシし、
`{"release": "vX.Y.Z", "pre_release": "vX.Y.Z-..."}` を返します
（どちらのフィールドも欠けることがあります）。結果は KV に
...
`POST /api/mail` は認証フィールドを JSON ボディで運び、その他は
すべて `X-Shyake-*` ヘッダーを使用します。
...
（`KEY_MISMATCH`、送信された受信者フィンガープリントがもはや
一致しない）、`410`（`USER_DESTROYED`）、`413`（ペイロードが
大きすぎる）、`403`（ブロック済み、不正な PoW、または古い
タイムスタンプ）。

#### 5.3 サイズ制限

サーバーは `POST /api/mail` の生の HTTP リクエストボディに
ハードキャップを課します。デフォルトは
**196608 バイト（192 KiB）**で、`wrangler.toml` の
`MAX_MAIL_SIZE` で設定可能です。絶対上限は 786432 バイト
（768 KiB）で、Cloudflare D1 の単一行制限によるものです。
...
クライアントは常にユーザー自身のインスタンスとのみ通信します。
リモートの受信者へ送信する際、クライアントは自身の送信者文字列を
...
1. クライアントは署名・暗号化済みペイロードを**送信者自身の
   インスタンス**に POST します（`POST /api/mail`）。
2. 送信者のインスタンスはメールをローカルの D1 データベースに
   保存します。
3. 同一リクエストのライフサイクル内で
   （`executionCtx.waitUntil` 経由）、サーバーは元の生ペイロード
   を `https://<recipientDomain>/api/mail` へ転送します。

受信者のインスタンスは、送信者のインスタンスから送信者の公開鍵を
取得して（`GET /api/pubkey/<sender>`）、送信者の署名を独立に
検証します。

送信者と受信者の両方のデータベースがメールを保存します。これに
より、リモートインスタンスの可用性に関わらず、送信者側の原子性
（送信ボックスの可用性）が保証されます。

#### 6.3 フェデレーションの切り替え

`wrangler.toml` の `FEDERATION_ENABLED` で設定できます。
`false` の場合、インスタンスはリモートユーザーの解決を拒否し、
受信のリレーメールと送信のインスタンス間送信の両方が拒否されます。

---

### 7. 信頼モデル（TOFU + OOB）

Shyake は公開鍵管理に **Trust On First Use（TOFU）** を
採用しています：

- **初回接触**：クライアントは `GET /api/pubkey/<recipient>` を
  問い合わせ、KEM フィンガープリントを計算し、
  `~/.config/shyake/known_hosts` に黙って追記します。
- **以降の接触**：送信のたびに、取得した鍵を `known_hosts` の
  エントリと比較します。不一致の場合、何も送信される前に
  ローカルで `KEY_MISMATCH` により中止します。
- **サーバー側のダブルチェック**：ペイロードに
  `recipient_kem_fingerprint` が埋め込まれており、保存済みの鍵と
  一致しなくなった場合、サーバーは独立に HTTP 409 で拒否します。
- **鍵ローテーションの検出**：クライアントは致命的エラーを表示
  して停止します：
...
`fingerprint` コマンドは**帯域外（OOB）検証**を提供します：
サーバーから現在の公開鍵を取得し、フィンガープリントを計算して
`known_hosts` と比較します。出力には GPG スタイルの 16 進
グループと OpenSSH スタイルの randomart イメージが表示されます。
`--update` フラグは、ユーザーが信頼できるチャネルで新しい
フィンガープリントを検証した後に `known_hosts` を書き換えます。

---

### 8. クライアントライブラリ ABI

コアライブラリは `include/shyake.h` を通じて安定した C API を
公開します。ABI の破壊を防ぐため、内部状態は不透明ポインタの
背後に隠されています：
...
内部構造体の定義は `src/lib/lib_internal.h` にあり、呼び出し側
には公開されません。セマンティックなエラーコードは型付き列挙型
（`shyake_err`）として返され、後方互換のため `SHYAKE_OK = 0`
です：
...
API グループ：コンテキストのライフサイクル、鍵生成、PoW 生成、
登録、メール（`shyake_send`、`shyake_check`、`shyake_fetch`、
`shyake_check_one`、`shyake_burn`）、ローカル保存メール
（`shyake_save_mail`、`shyake_read_saved`、
`shyake_check_saved_one`、`shyake_list_saved`）、アカウント
（`shyake_block`、`shyake_rotate`、`shyake_destroy`）、
フィンガープリント（`shyake_fingerprint`）、単体ファイル暗号化
（`shyake_enc_file`、`shyake_dec_file`）、自己更新
（`shyake_get_latest_version`、`shyake_version_cmp`、
`shyake_self_update`）。

共有ライブラリ（`libshyake.so` / `libshyake.dylib`）は
サードパーティの FFI 利用者向けです。CLI バイナリは単一ファイル
配布のため、静的アーカイブ（`libshyake.a`）にリンクされます。
...
`--no-header` を受け付けます。`send` の宛先はローカル
（`username`）またはリモート（`username@instance`）を指定でき、
バイナリデータは呼び出し側で base64 エンコードする必要が
あります。`enc`/`dec` はデバッグとテスト用途を想定しています。
...
`CompiledWasm` ビルドルールが `mldsa65-wasm` モジュールを
ロードします。
...
（通常リリース）と **preview**（プレリリース）。サーバー
エンドポイント `GET /api/client/version` は GitHub Releases API
をプロキシし、各チャネルの最新タグを選択して、結果を KV に
1 時間キャッシュします。

`shyake update` はこのエンドポイントを取得し、semver 順序
（`vX.Y.Z`；同じベースバージョンではリリースがプレリリースより
上位）でタグを比較します。preview チャネルは stable より新しい
場合にのみ提示されます。

`shyake update stable|preview` は自己更新を実行します：

1. OS／アーキテクチャに一致するリリースアセット
   （`shyake-<os>-<arch>.tar.gz`）と `sha256sums.txt` を
   GitHub Releases からダウンロードする。
2. アーカイブの SHA-256 をチェックサムファイルと照合し、
   不一致なら中止する。
3. アーカイブを展開し、実行中のバイナリをその場で置き換える
   （パスは `/proc/self/exe`、`_NSGetExecutablePath`、または
   `which shyake` で解決）。
